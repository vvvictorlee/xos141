...
using controller_index_set = index_set<
   ...
   global_property_list_multi_index,
...
///xos begin
 void initialize_database_xos() {
      // Initialize block summary index
      for (int i = 0; i < LIST_TYPE_COUNT; i++)
         db.create<global_property_list_object>([&](global_property_list_object& plo) {plo.list_type=i});

  
  
      // conf.genesis.free_resource_limit_per_day.validate();
      db.create<global_property_object_ex>([&](auto& gpo ){
        // gpo.free_resource_limit_per_day = conf.genesis.free_resource_limit_per_day;
      });

 }


 void add_list_xos(uint16_t type,const account_name_list& list) {
      chain_list_config config;
      config.list =list;
   const auto &property_list = db.get<global_property_list_object, by_type>(type);
      _db.modify(property_list, [&](auto &pl) {
      pl.add_list(config);
    });

 }

void remove_list_xos(uint16_t type,const account_name_list& list) {
      chain_list_config config;
      config.list =list;
   const auto &property_list = db.get<global_property_list_object, by_type>(type);
      _db.modify(property_list, [&](auto &pl) {
      pl.remove_list(config);
    });
 }

 void set_string_list_xos(string& type,string& action,const string& list) {
      chain_list_config config;
      config.list =list;
   const auto &property_list = db.get<global_property_list_object, by_type>(type);
      _db.modify(property_list, [&](auto &pl) {
 ...
 void add_string_list_xos(uint16_t type,const string& list) {
   const auto &property_list = db.get<global_property_list_object, by_type>(type);
      _db.modify(property_list, [&](auto &pl) {
      pl.add_string_list(list);
    });

 }

void remove_string_list_xos(uint16_t type,const string& list) {
   const auto &property_list = db.get<global_property_list_object, by_type>(type);
      _db.modify(property_list, [&](auto &pl) {
      pl.remove_string_list(list);
    });
 }
///xos end
   void initialize_database() {
     initialize_database_xos();
   ...
   void finalize_block()
   {
      ...
      */
      ...
      resource_limits.set_block_parameters(
        ...
        resource_limits.set_block_parameters_xos(config::default_free_resource_limit_per_day );
